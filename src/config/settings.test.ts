...
describe('registerOpenSettingsCommand', () => {
	it('should register the open settings command and execute it correctly', async () => {
		...
		expect(mockCommandAdapter.registerCommand).toHaveBeenCalledWith('envsync-le.openSettings', expect.any(Function))

		// Get the callback function that was registered
		const registeredCallback = (mockCommandAdapter.registerCommand as vi.Mock).mock.calls[0][1]
