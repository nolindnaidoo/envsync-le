...
export function createVSCodeConfiguration(workspace: MinimalWorkspace): Configuration {
	...
	return {
		...
		getSection(section: string): Configuration {
			const sectionConfig = config.get(section) as Record<string, unknown> | undefined
			return {
				get<T>(key: string, defaultValue: T): T {
