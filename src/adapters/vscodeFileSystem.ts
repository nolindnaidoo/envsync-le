...
interface VSCodeDependencies {
	...
	findFiles: (pattern: string, exclude?: string | null, maxResults?: number) => Promise<Uri[]>
...
export function createVSCodeFileSystem(deps: VSCodeDependencies): FileSystem {
	return {
		async findFiles(pattern: string, exclude?: string | null, maxResults?: number): Promise<FileInfo[]> {
