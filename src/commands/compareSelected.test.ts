...
describe('compareSelected command', () => {
	...
	beforeEach(() => {
		...
		detector = {
			checkSync: vi.fn(),
			checkSyncForFiles: vi.fn().mockResolvedValue(undefined as any),
			dispose: vi.fn(),
		};
		fileSystem = {
			findFiles: vi.fn(),
	...
	function getHandler() {
		registerCompareSelectedCommand(context, {
	...
	it('invokes detector with two selected env files', async () => {
		...
		expect(detector.checkSyncForFiles).toHaveBeenCalledWith([
			'/root/.env',
			'/root/.env.local',
		]);
	...
	it('palette flow: picks env files and runs compare', async () => {
		...
		(ui.showQuickPick as any).mockResolvedValue([
			'/root/.env',
			'/root/.env.local',
		]);
		await handler();
