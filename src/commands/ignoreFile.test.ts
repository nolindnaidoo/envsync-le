...
describe('ignoreFile command', () => {
	...
	function getHandlers() {
		...
		const ignore = calls.find(
			(c: any[]) => c[0] === 'envsync-le.ignoreFile',
		)[1];
		const stop = calls.find(
			(c: any[]) => c[0] === 'envsync-le.stopIgnoring',
		)[1];
		const clear = calls.find(
			(c: any[]) => c[0] === 'envsync-le.clearAllIgnored',
		)[1];
	...
	it('adds a file to temporaryIgnore when not already ignored', async () => {
		...
		await ignore(target);
		expect(cfgObj.update).toHaveBeenCalledWith(
			'temporaryIgnore',
			['.env'],
			expect.anything(),
		);
