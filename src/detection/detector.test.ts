...
import type { Configuration, FileSystem, Notifier, StatusBar, Telemetry } from '../interfaces'
...
const createMockConfiguration = (): Configuration => ({
	...
	getSection: vi.fn(() => ({
		...
		getSection: vi.fn(() => ({ get: vi.fn(), getSection: vi.fn(), has: vi.fn(), keys: vi.fn() })),
		has: vi.fn(() => false),
		keys: vi.fn(() => []),
	})),
	has: vi.fn(() => false),
...
describe('createDetector', () => {
	...
	it('should return disabled status when configuration is disabled', async () => {
		...
		vi.mocked(configuration.get).mockImplementation((key: string, defaultValue: unknown) => {
	...
	it('should discover and compare files when enabled', async () => {
		...
		vi.mocked(fileSystem.readFile).mockImplementation((filepath: string) => {
			if (filepath === '.env') return Promise.resolve('KEY1=value1\nKEY2=value2')
		...
		vi.mocked(fileSystem.asRelativePath).mockImplementation((filepath: string) => filepath)
		...
		expect(notifier.showMissingKeys).toHaveBeenCalledWith('.env.local', ['KEY2'])
	...
	it('should handle file reading errors gracefully', async () => {
		...
		// Mock file discovery
		vi.mocked(fileSystem.findFiles).mockResolvedValue([{ filepath: '.env', uri: 'file://.env' }])

		// Mock file reading to throw error
		vi.mocked(fileSystem.readFile).mockRejectedValue(new Error('File read error'))
	...
	it('should handle errors during sync check', async () => {
		...
		vi.mocked(fileSystem.findFiles).mockRejectedValue(new Error('Discovery error'))
		...
		expect(result.errors[0].message).toContain('Failed to check sync')
		expect(result.errors[0].filepath).toBe('workspace')

		// Verify error handling UI updates
		expect(statusBar.updateStatus).toHaveBeenCalledWith('parse-error', 0)
		expect(notifier.showError).toHaveBeenCalledWith('Failed to check dotenv sync: Discovery error')
	})
	...
	it('should apply comparison mode filtering correctly', async () => {
		...
		expect(result.files).toHaveLength(2)
		expect(result.files.map((f) => f.path)).toEqual(['.env', '.env.production'])
	...
	it('should compare against template keys when in template mode', async () => {
		...
		const detector = createDetector({ telemetry, notifier, statusBar, configuration, fileSystem })
		const result = await detector.checkSync()
