...
describe('shouldExcludeFile', () => {
	...
	it('should handle complex paths', () => {
		...
		expect(shouldExcludeFile('deep/nested/path/.env.development.local', patterns)).toBe(true)
	})

	it('should match path-aware patterns', () => {
		const patterns = ['packages/**/.env.*.local']
		expect(shouldExcludeFile('packages/app/.env.production.local', patterns)).toBe(true)
		expect(shouldExcludeFile('packages/web/nested/.env.dev.local', patterns)).toBe(true)
...
describe('parseDotenvFile', () => {
	...
	it('should handle malformed content gracefully', () => {
		...
		// The dotenv parser is lenient and treats this as valid (empty keys)
		expect(result.success).toBe(true)
		expect(result.keys).toEqual([])
		expect(result.errors).toEqual([])
	})
